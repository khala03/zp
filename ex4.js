...
if (magaz=='232'){
    ...
    checkZp();
        ...
        alert('Введен неверный пароль для H-232')
    }
}
else if (magaz=='102'){
    pass=prompt('Введи пароль для для H-102 ТЦ "ВиВа":');
    if (pass=='1102'){
    checkGreat102(); 
    checkZp();
    }
    else{
        alert('Введен неверный пароль для H-102')
    }
...
/*--------Подарочные карты-------*/
function checkPK(){
    pksum=prompt('Общая сумма подарочных карт? (Пример: 25000)');
    if (pksum<10000){
        pksum=(0);
        alert('За продажу ПК выплат не будет, т.к. сумма продаж ПК меньше 10000руб');
    }
}
...
function pereProcYes(){
    pereproc=prompt('Какой процент выполнения был засчитан?')
}
...
function checkZp(){
    if (dol=='upr' || dol=='zam'){
    zp=((okl+disA+disob)*0.87+prem+(nightchas*poch*0.87)+(perer*poch*0.87)+pksum*0.01);
    console.log('peme = '+zp)
    ...
    zp=((okl+disA+disob)*0.87+(prem*reyt)+(nightchas*(poch*0.87))+(perer*(poch*0.87))+pksum*0.01);
    zap=parseFloat(zp.toFixed(0));
